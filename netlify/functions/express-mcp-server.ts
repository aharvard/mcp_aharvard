...
app.post("/mcp", async (req: Request, res: Response) => {
    ...
    console.log("⚾️Received POST MCP request", { body: req.body });
...
app.get("/mcp", (_req: Request, res: Response) => {
    ...
    console.log("⚾️Received GET MCP request");
    res.status(405).json({
        jsonrpc: "2.0",
        error: {
            code: -32000,
...
app.delete("/mcp", (_req: Request, res: Response) => {
    console.log("⚾️Received DELETE MCP request");
    res.status(405).json({
        jsonrpc: "2.0",
        error: {
            code: -32000,
